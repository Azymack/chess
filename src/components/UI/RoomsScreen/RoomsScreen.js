...
const RoomsScreen = () => {
  ...
  useEffect(() => {
    const skt = io.connect(
      `http://${process.env.REACT_APP_SERVER_URL}:${socketServerPort}`
    );
    setSocket(skt);

    skt.on(socketEvents["SC_RoomCreated"], (params) =>
    ...
    <div className="rooms">
      <div className="rooms-container">
        ...
        <div className="rooms-container-content">
          <p className="rooms-container-content-desc">
            There are 5 different rooms. Two players each put in the specified
            amount of LLG (based on the room type) into a pool. After each game,
            whoever wins the game takes all the LLG in the pool.
          </p>
          <div className="rooms-container-content-rooms">
            {ROOMS.map((room, index) => (
              <div
                className={`rooms-container-content-room ${
                  room.name !== "Classic Room" ? "closed" : ""
                }`}
                key={index}
